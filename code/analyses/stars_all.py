...
def main(path_data, path_data_figures, experiments, out_folder_name, bootstrap_reps):
    thresholds = (-0.5, 0.5)
    ...
    for profile in probas_exp:
        for i, P in ((0, "P0"), (1, "P1234"), (5, "P5")):
            np.savetxt(
                path_data_figures / out_folder_name / f"exp/classification/{profile}/{P}.txt",
                np.column_stack(
                    (
                        values,
            ...
            np.savetxt(
                path_data_figures / out_folder_name / f"model/classification/{profile}/{P}.txt",
                np.column_stack(
                    (
                        values_model,
                        probas[profile][:, i],
    ...
    with open(path_data_figures / out_folder_name / "model/parameters/stars.json", "w") as file:
        json.dump(
            {
                "col": {
                    "functionType": "tanh",
