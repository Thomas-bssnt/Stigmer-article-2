...
#pragma omp critical
    ...
    for (auto iAgent : m_iAgents)
    {
        ...
        computeScore(iGame, game, iAgent);
        computeRank(iGame, game, iAgent);
...
void GameAnalyzer::saveObservables(std::string pathObservables) const
{
    ...
    saveObservable(pathObservables + "MNS", get_MNS());
...
void GameAnalyzer::computeScore(int iGame, const Game &game, int iAgent)
{
    ...
    m_S[m_playerCount] = normalizedScore;
    m_S_group[iGame] += normalizedScore / m_numberOfPlayersToAnalyze;
}

void GameAnalyzer::computeRank(int iGame, const Game &game, int iAgent)
{
    m_rank[m_playerCount] = 1;
